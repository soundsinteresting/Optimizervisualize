...
def loss_surface_and_trajectory(name_list0, name_list1, sample_net, test_loader, grid_x=100, grid_y=100, device='cpu'):
    nop = len(name_list0)
    ...
    for i in range(grid_x):
        for j in range(grid_y):
            netij = mo.scalar_mul([1,2*(i-grid_x/2)/grid_x*ld, 2*(j-grid_y/2)/grid_y*ld],[center, e_1, e_2])
            loss_sur[i][j] = loss_value(netij, test_loader, device)
            loss_grid_x[i] = 2*(i-grid_x/2)/grid_x
            loss_grid_y[j] = 2*(j-grid_j/2)/grid_j
...
def load_and_create(source_folder_name, sample_net, test_loader, target_folder_name, device):
    ...
    for name in all_files:
        ...
        if s:
            ...
            if len(betatwo) == 0:
                betatwo.append(b2)
            if len(beta2) == 1 and betatwo[0] != b2:
                betatwo.append(b2)
            if betatwo[0] == b2:
    ...
    surface, x, y, gx, gy = loss_surface_and_trajectory(name_list0, name_list1, sample_net, test_loader, 10, 10, device)
    ...
    np.savetxt(target_folder_name + 'y_grix.txt', gy)
